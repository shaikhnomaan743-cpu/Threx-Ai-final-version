  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:20}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
        <div className="glass glow-card"><div className="panel-head"><h3>Top DGA Sources</h3></div><div className="panel-body">
          {d.families.map((f:{name:string;count:number})=>(<div key={f.name} style={{display:'flex',justifyContent:'space-between',padding:'10px 0',borderBottom:'1px solid var(--color-border)',fontSize:13}}>
            <span style={{display:'flex',alignItems:'center',gap:6}}><AlertTriangle size={12} style={{color:'var(--color-amber)'}}/>{f.name}</span>
            <span className="num" style={{fontSize:12,color:'var(--color-text-dim)'}}>{f.count} detections</span></div>))}
        </div></div>
        <div className="glass glow-card"><div className="panel-head"><h3>Flagged Domains</h3></div><div style={{overflowX:'auto'}}>
          <table className="tbl"><thead><tr><th>Query Target</th><th>Entropy</th><th>Severity</th><th>Confidence</th></tr></thead><tbody>
