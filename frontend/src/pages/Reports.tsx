...
export default function Reports(){
  ...
  const go=async()=>{
    setGen(true); setDone(null); setErr(null);
    try {
      const { blob, filename, count } = await generateReport({ format: fmt, cls, sev, limit: lim });
      ...
      setDone(`${filename} · ${count} records`);
    } catch (e) {
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:20}}>
      ...
      <div className="glass glow-card" style={{maxWidth:520,padding:24,display:'flex',flexDirection:'column',gap:18}}>
        ...
        {done&&<div className="fade-in" style={{fontSize:12,color:'var(--color-green)'}}>✓ Downloaded {done}</div>}
        {err&&<div className="fade-in" style={{fontSize:12,color:'var(--color-red)'}}>✗ {err}</div>}
      </div>
    </div>
  );
}
