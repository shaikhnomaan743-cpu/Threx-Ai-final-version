...
const CLS: ThreatClass[] = ['DDoS','C2 Beaconing','DGA','DNS Tunnel','TLS Malware','Recon','Exfiltration'];
...
export default function Threats() {
  ...
  const filtered = data.filter(t=>{
    ...
    if(q&&!t.source_ip.includes(q)&&!t.destination_ip.includes(q)&&!t.threat_class.toLowerCase().includes(q.toLowerCase())&&!t.flow_id.includes(q)) return false;
    return true;
  });
            ...
            <tbody>
              {filtered.map((a,i)=>(
                <tr key={a.id} className={i<3?'slide-in':''}>
                  ...
                  <td className="num" style={{fontSize:11,color:'var(--color-green)'}}>{a.block_height ? '#'+a.block_height.toLocaleString() : '—'}</td>
