...
export class SearchIndex {
  ...
  /** Add an alert id to the set for this IP, creating the set on first sight. */
  private indexIp(ip: string, alertId: string) {
    if (!ip) return;
    const existing = this.ips.get(ip);
    if (existing) existing.add(alertId);
    else this.ips.set(ip, new Set([alertId]));
  }
  ...
  build(alerts: ThreatAlert[]) {
    ...
    for (const alert of alerts) {
      this.indexIp(alert.sourceIp, alert.id);
      this.indexIp(alert.destinationIp, alert.id);
