...
async def generate_report(
    ...
    # When called directly rather than through FastAPI, `report_type` is the
    # Query(...) default object, not None. Coerce it before any use.
    _rt = report_type if isinstance(report_type, str) else None
    report_type = _rt
    ...
    response = {
        "report_id": report_id,
        "status": "generated",
        "type": cfg.threat_class or cfg.report_type or _rt or "threat_summary",
        "format": fmt,
...
async def generate_report_get(
    ...
    return await generate_report(cfg=cfg, report_type=None, alert_manager=alert_manager)  # type: ignore
